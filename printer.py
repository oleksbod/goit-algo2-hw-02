...
def optimize_printing(print_jobs: List[Dict], constraints: Dict) -> Dict:
    ...
    jobs.sort(key=lambda x: x.priority)  # Сортування за пріоритетом
    ...
    for job in jobs:
        if current_volume + job.volume <= printer_constraints.max_volume and len(current_group) < printer_constraints.max_items:
            current_group.append(job)
            current_volume += job.volume
        else:
            if current_group:
                max_time = max(j.print_time for j in current_group)
